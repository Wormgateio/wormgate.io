...
function MultipleMintForm({ bridgePriceList, bridgeType, updateBridgePrice, onSubmit }: MultipleMintFormProps) {
  ...
  const clearAllChains = () => {
    form.setFieldValue(
      'to',
      watchedFormData.to.map((c) => ({ ...c, checked: false }))
    );
  };
    ...
    <Form size="large" layout="vertical" form={form} onFinish={onSubmit}>
      <Flex className={cn.wrapper}>
        ...
        <div className={cn.chainsWrapper}>
          <button className={cn.clearButton} onClick={clearAllChains} type="button">
            Clear All
          </button>
