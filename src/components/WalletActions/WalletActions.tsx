...
function WalletActions() {
  ...
  if (!isClient) {
    return (
      <Flex justify="end">
        <Button className={styles.btn}>Connect a Wallet</Button>
      </Flex>
    );
  }
  ...
  return (
    <div className={styles.root}>
      {!isConnected ? (
        <Button onClick={openAccountDrawer}>{isConnecting ? 'Connecting...' : 'Connect Metamask'}</Button>
      ) : (
        <Flex gap={19}>
            ...
            <div>
              {address && (
                <button className={styles.btn} onClick={openAccountDrawer}>
                  {account?.twitter.user?.username ? account.twitter.user.username : <AccountAddress address={address} />}
                </button>
              )}
            </div>
          </div>
